...
def extract_hotel_performance_train(train_df):    
    ...
    # Compute features, leaving out current row (to avoid bias)
    # Apply bayesian smoothing: https://en.wikipedia.org/wiki/Bayesian_average 
    minusone = total_count - 1
    C_bayesian = 30

    train_df['hotel_booking_rate'] = ((total_bookings - train_df['booking_bool']) + C_bayesian * global_booking_rate ) / (minusone + C_bayesian)
    train_df['hotel_click_rate'] = ((total_clicks - train_df['click_bool']) + C_bayesian * global_click_rate) / (minusone + C_bayesian)
    train_df['hotel_avg_position'] = ((total_position - train_df['position']) + C_bayesian *  global_position_avg)/ (C_bayesian + minusone)
    train_df['hotel_n_appearances'] = minusone
